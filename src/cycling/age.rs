...
//! athletes (plateau into the mid-30s, then about **0.5% per year** / ~5% per
//! decade for aerobic power). See Tanaka & Seals and related endurance-aging
//! literature. **Not** official age grading; gender is unused.
//!
...
fn decline(age: u16) -> f64 {
    let years_after = age.max(AGE_PLATEAU).saturating_sub(AGE_PLATEAU) as i32;
    (1.0 - AEROBIC_DECLINE_PER_YEAR).powi(years_after)
}
